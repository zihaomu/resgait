...
class Test(object):
    ...
    def load_new_label(self):
        data = pd.read_csv("./data/label.csv") # fit right path of "label.csv"
    ...
    # please check the file at "./utils/ROC_EER.py"
    ...
    def run(self):
        ...
        for kk in iterater:
            ...
            for seq, date, label, _ in self.data_loader:
                ...
                pre_cloth.extend(torch.max(out_cloth, 1)[1].detach().cpu().numpy())
                pre_activity.extend(torch.max(out_activity,1)[1].detach().cpu().numpy())
                pre_gender.extend(torch.max(out_gender, 1)[1].detach().cpu().numpy())
                pre_carry.extend(torch.max(out_carry, 1)[1].detach().cpu().numpy())
                pre_path.extend(torch.max(out_path, 1)[1].detach().cpu().numpy())
            ...
            pre_cloth, pre_activity, pre_gender, pre_carry,pre_path, label_cloth, label_activity, label_gender,label_carry, label_path = map(transform_to_numpy,[pre_cloth, pre_activity, pre_gender, pre_carry,
                                                pre_path,label_cloth,label_activity, label_gender,
                                                label_carry, label_path])

            acc_cloth = np.sum(pre_cloth == label_cloth) / float(len(label_cloth))
            acc_activity = np.sum(pre_activity == label_activity) / float(len(label_cloth))
            acc_gender = np.sum(pre_gender == label_gender) / float(len(label_cloth))
            acc_carry = np.sum(pre_carry == label_carry) / float(len(label_cloth))
            acc_path = np.sum(pre_path == label_path) / float(len(label_cloth))
            ...
            print('acc_cloth', acc_cloth, 'acc_activity', acc_activity, 'acc_gender',acc_gender,'acc_carry',acc_carry,'acc_path',acc_path)
